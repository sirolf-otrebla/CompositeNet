...
from networks.network_seg_fusion import NetFusion as NetFusion
...
def get_model(model_name, input_channels, output_channels, args):
    ...
    return Net(input_channels, output_channels, config=config, args=args)
...
def train(args, flist_train, flist_test):
    ...
    net_fusion = NetFusion(input_channels=2 * 128, output_channels=N_CLASSES, config=config)
    net_rgb.cuda()
    net_noc.cuda()
    net_fusion.cuda()
    net_rgb.eval()
    ...
    train_loader = torch.utils.data.DataLoader(ds, batch_size=config["batchsize"], shuffle=True,
    ...
    test_loader = torch.utils.data.DataLoader(ds_val, batch_size=config["batchsize"], shuffle=False,
                                              num_workers=args.threads
                                              )
    ...
    root_folder = os.path.join(args.savedir,
                               "{}_area{}_{}_fusion_{}".format(args.model, args.area, args.npoints, time_string))
